...
- 保护代码块
- 修改Hexo渲染源码
但是都存在着一些问题，手动escape需要对公式涉及到的每一个markdown关键词转义，例如：`$$f'(x\_0)=\lim_{\Delta x\to 0} \frac{f(x\_0+\Delta x) - f(x\_0)}{\Delta x}$$`，`$$f'(x\_0)=\lim_{\Delta x\to 0} \frac{f(x\_0+\Delta x) - f(x\_0)}{\Delta x}$$首先是没办法在markdown中预览到公式，其次这样每一个关键词都要转义实在是太繁琐了。
