...
- 保护代码块
- 修改Hexo渲染源码
但是都存在着一些问题，手动escape需要对公式涉及到的每一个markdown关键词转义，例如：`$$f'(x\_0)=\lim_{\Delta x\to 0} \frac{f(x\_0+\Delta x) - f(x\_0)}{\Delta x}$$`，首先是没办法在markdown中预览到公式（有些markdown编辑器可以），其次这样每一个关键词都要转义实在是太繁琐了。最后的修改hexo的渲染源码太过于激进，且先不说修改代码会带来不可预料的bug，日后的升级维护也会带来很多麻烦。最终找到了pandoc的这个好东西，pandoc是什么介绍
##4.CI-自动构建
为什么要自动构建
